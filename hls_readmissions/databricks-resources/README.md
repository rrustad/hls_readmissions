...
The databricks CLI bundles top level is defined by file `hls_readmissions/bundle.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`hls_readmissions/databricks-resources/model-workflow-resource.yml`)
 - batch inference workflow (`hls_readmissions/databricks-resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`hls_readmissions/databricks-resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`hls_readmissions/databricks-resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`hls_readmissions/databricks-resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` environment is used by ML engineers to deploy ML resources to development workspace with `dev` environment configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-hls_readmissions-model     | /dev-hls_readmissions-experiment     |
| staging     | The `staging` environment is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` environment config.                                                             | staging workspace    | staging-hls_readmissions-model | /staging-hls_readmissions-experiment |
| prod        | The `prod` environment is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` environment config.                                                                      | prod workspace       | prod-hls_readmissions-model    | /prod-hls_readmissions-experiment    |
| test        | The `test` environment is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` environment. | staging workspace    | test-hls_readmissions-model    | /test-hls_readmissions-experiment    |
...
2. Complete the `TODO` in `hls_readmissions/bundle.yml` to add the dev workspace URI under `environments.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `hls_readmissions/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `hls_readmissions/databricks-resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.environment}-hls_readmissions-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.environment}-hls_readmissions-batch-inference-job`
that runs the notebook under `hls_readmissions/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.environment}-hls_readmissions-batch-inference-job`. 
...
`test-hls_readmissions-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-hls_readmissions-batch-inference-job`.
